...
function fnAdtnInfo(obj){
	...
	url = "/jsp/u01/u12/U12_01_30_0.jsp?"+param;

//	result = openModalNs(url, frm_main, "dialogWidth:800px; dialogHeight:410px; scroll:no");
	result = openModal(url, frm_main, "dialogWidth:800px; dialogHeight:570px; scroll:no");

	if(typeof result != "undefined"){
		if(result.length > 20){
			$("#ItxpAdtnInfo_"+tObjIdx).html('<img src="/images/bt/bt_regist4.gif" alt="���">');
	...
	if( $('#EvmaGb_'+objIdx).val() == "E" ){
		for(var i = 0 ; i < EtcImplItxpCd.length; i++){
			if( EtcImplItxpCd[i] == document.frm.SelSubDtlsItxpCd.value){
				...
				$('#EtcEvdcExcCd_'+objIdx).val('');
				$('#EtcEvdcExcNo_'+objIdx).val('');
			}
		}//end of for
	}else{
		$('#EtcEvdcExcCd_'+objIdx).val('');
...
function fnDelete(obj, idx){
	...
	if( chk == true ){
		if(!confirm("["+ name +"] ���κ������ �����\n�ΰǺ� ��ϳ����� ���� �˴ϴ�.\n�����Ͻðڽ��ϱ�?")){
			...
			for(i = 0 ; i <= rows ; i++ ){
				// �ΰǺ� ���� ���� �ʱ�ȭ.
				jQuery("#ItxpAdtnInfo_"+i).html('<img src="/images/bt/bt_nonregist.gif" alt="�̵��">');
				$("#rchrInfo_"+i).val("");
			}
		}
	}
 ...
 ******************************************************************/
 uf_addEvdc_pop = function (frm, evma_gb, row_idx){
 	...
 	if(evma_gb=="T"){ //���ڼ���
 		...
 		url = "/jsp/u01/u12/U12_01_10_0.jsp?"+param;
 	}else if(evma_gb=="C"){ //ī��
 		pop_width = "760";
		pop_height = "800";
 		pop_scroll = 'yes';
		...
		url = "/jsp/u01/u12/U12_01_40_0.jsp?"+param;
 	}else if(evma_gb=="E"){
 		return;
 	}else{
 		//���켱 �ʱ�ȭ
 ...
 chk_itxp_evdc_amt = function(){
 	...
 	for(var i = 0; i < document.forms['frm'].elements['itxp_cnt'].value ; i++){
 		...
 		if(( "C" == $('#EvmaGb_'+i).val())){
	 	 	...
	 	 	//2014.08.13 kbm : ī�������� �����Գ����� �ݾ׺��� ���Ϸ� ��� ����
 	 		if( $('#C_Evdc_SplyAmt_'+i).val() == tot_splyAmt )
 	 		{
	 	 		if(tot_useAmt > $('#C_Evdc_SumAmt_'+i).val() ){
	 	 			return "����� ���ݾ��� ��ϵ� ������ ���ݾ�("+toMoney($('#C_Evdc_SumAmt_'+i).val())+")�� �ʰ��� �� �����ϴ�.";
	 	 		}else if(tot_splyAmt > $('#C_Evdc_SumAmt_'+i).val() ){
	 	 			return "����� ���ޱݾ��� ��ϵ� ������ �� �ݾ�("+toMoney($('#C_Evdc_SplyAmt_'+i).val())+")�� �ʰ��� �� �����ϴ�.";
	 	 		}
	 	 		//2014.08.13 kbm : ī���� ��� �ΰ����ݾ��� �����Գ��� �ݾ��� �ŷ��� �� ���� ������ ����� ������ ����(�����Ժΰ����ݾ׺��� ũ�� ��� ����)
	 	 		/*
...
function fnAddRow(upd_flag){
	...
	try{
		...
		newRow1.addClass('blue_line').find('.btn-opendtl > img').attr('src','/images/bt/bt_grid1.gif').attr('alt','�߰��Է�');
		...
		newRow3.find('#etc_SplrCoNm_' 		+ rows).css('backgroundColor','#fff').attr('readonly',false).attr('disabled',false);
		newRow3.find('#etc_SplrBzRegNo_' 	+ rows).css('backgroundColor','#fff').attr('readonly',false);
		newRow3.find('#etc_ChkBzRegNo_' 	+ rows).html('<img src="/images/bt/bt_bzregno.gif" alt="����ڹ�ȣȮ��"/>');
		...
		jQuery(jRowObj).find("span").each(function(){
			...
			if(id){
				...
				if( _id=="ItxpAdtnInfo_"+rowCnt ){
					jQuery(this).html('<img src="/images/bt/bt_nonregist.gif" alt="�̵��">');
 ...
 * @return
******************************************************************/
uf_SelfInfoInp = function(InstNm, BzRegNo, TpopCd, TpbzCd, LoctDtlsAdr, ReprNm){
	if($('#etc_SelfInfoInp_0').is(":checked")) {
...
//	// ���ΰŷ� ���� üũȮ��
//	if($('#etc_SelfInfoInp_0').is(":checked")) {
//		$('#etc_SelfInfoInp_0').attr('checked', false); // �ڱ������Է� üũ����
...
setSplrPrsn = function(frm,frm_key, ridx){
	...
	if( "B0201" == $("#SelDtlsItxpCd").val() ){//�����ü������ �� ����� ���ΰŷ� �Ұ�
		//2014.11.27 ������ ���� : 2013-12-27 ����Ǯ�� ������ Ȯ�εǳ�, �� Ǯ�ȴ��� Ȯ�κҰ�.
		//					    ���켱��� �� ���� ���� ���� �� ���ΰŷ�üũ �����ϵ��� �Ǿ��־� ���ΰŷ� üũ �ȵǵ��� ����
		prsn = false;
	}
	...
	for(var i=0; i<frm.itxp_cnt.value; i++){
		...
		if( $('#EvmaGb_' + i).val() == 'E' ){ //��Ÿ������ �ش�
			...
			setChkBzRegNo('1',frm_key.S_NPRF_INST_YN.value,frm_key.S_OPAG_YN.value,i);
		}else if( $('#EvmaGb_' + i).val() == 'T' ||  $('#EvmaGb_' + i).val() == 'R'){
			setChkBzRegNo('1',frm_key.S_NPRF_INST_YN.value,frm_key.S_OPAG_YN.value.value,i);	//Add CashReceipt 20141216 modified by sonpro
...
/**
 * @Method Name : uf_etcEvdcExcCdNull
 * @Date : 2015. 1. 12.
 * @Description :��Ÿ�����Ұ��� ���(�����ü�.��������)�� ��� ��Ÿ���������ڵ� �ʼ�üũ
 * @history :
